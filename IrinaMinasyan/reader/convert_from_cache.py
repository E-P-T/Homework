...
class ConvertCache:
    ...
    def convert_news_in_html(news, html_file):
        ...
        with html_file:
            with tag.div():
                tag.h2(news[0].get_title())
                tag.p(news[0].get_date())
                tag.br()
                tag.a("Read the full article", href=news[0].get_link())
                tag.br()
                tag.p(news[1])
                tag.br()
    ...
    def convert_news_in_pdf(pdf, news):
        ...
        pdf.multi_cell(200, 5, txt=news[0].get_title(), ln=True)

        pdf.set_font('DejaVu', '', 8)
        pdf.set_text_color(0, 150, 250)
        pdf.cell(100, 5, txt=news[0].get_date(), ln=True)

        pdf.set_font('DejaVu', 'U', 10)
        pdf.set_text_color(0, 50, 200)
        pdf.cell(200, 10, txt="Read the full article", link=news[0].get_link(), ln=True)

        pdf.set_font('DejaVu', '', 8)
        pdf.set_text_color(0, 150, 150)
        pdf.multi_cell(400, 5, txt=news[1], ln=True)
        pdf.ln(5)
