...
JINJA_ENV = Environment(
    loader=PackageLoader(package_path="./", package_name="rss_reader"),
...
PARSER.add_argument(
    "--to-html", type=str, dest="to_html", default=None, help="Convert news format to html document!"
)
            ...
            for elem in item:
                try:
                    if "content" in elem.tag:
                        single_news["image"] = elem.attrib["url"]
                    else:
...
def create_html_template(news, path, verbose):
    template = JINJA_ENV.get_template("news_template.html")
    if verbose:
        print("LOG: Loading html template from filesystem")
    with open(path, 'w+') as html_file:
        html_file.write(template.render(news=news))
    if verbose:
        print("LOG: Wrote content to html file!")
...
def main():
    ...
    if not json_output:
        ...
        pprint(format_json(news))
    if html_path:
        create_html_template(news, html_path, verbose)
        print(f"Successfully wrote news to the {html_path}")
